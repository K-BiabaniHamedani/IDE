...
switch Name
    ...
    case 'Dome1180Bar_Frequency'
        Problem_name = 'Dome1180Bar_Frequency';
        ndim = 59;                  % Number of design variables
        lb = 0.0001.*ones(1, ndim); % Lower bound of design variables
        ub = 0.01.*ones(1, ndim);   % Upperbound of design variables
        MaxNFEs = 30000;            % Maximum number of objective function evaluations
        nobs = 30;                  % Population size
        Maxt = round(MaxNFEs/nobs); % Maximum number of Iterations
        nr = 20;                    % Number of independent runs
        FitFunc = @(x, NFEs) fobj_Dome1180Bar_Frequency(x, NFEs, MaxNFEs); % Objective function
